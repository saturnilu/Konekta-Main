...
Services never touch `req`/`res` and instead take plain arguments and return plain objects, which makes them independently testable and reusable (e.g. `subscriptionService.getBalance()` is called both from the HTTP controller and internally from `requestWithdrawal()`).

### 2. Centralized Error Handling

**Where:** `middlewares/error.ts`, `utils/apiError.ts`

**Why:** every controller's `catch (e) { next(e); }` funnels into one Express error-handling middleware. A service throws `new ApiError(404, 'Offer not found')` and the middleware turns that into a consistent `{ success: false, message }` JSON response with the right status code, so no controller repeats that formatting logic.
...
**Where:** `services/subscription.service.ts`, specifically `_expireInfluencerIfNeeded()` / `_expireBrandIfNeeded()`
...
A unique key on `(influencer_user_id, platform, handle)` makes "duplicate" well-defined, and without it, double-tapping "Add" in the app used to silently create multiple rows for the same account.
...
Adding a third platform (YouTube Shorts, say) means writing one more provider file with a `fetchXStats()` matching this shape and adding one branch here, and `video.controller.ts`, `refreshAllVideosForUser()`, and the Flutter client never need to change, since they only ever see the normalized `VideoStats` shape.

### 9. Fail-Fast Startup Guard

**Where:** backend config/startup (checked when `NODE_ENV=production`)

**Why:** a JWT signed with a missing or weak secret is a silent security hole, since the server would start fine and *look* like it's working, right up until someone forges a token. Rather than let that happen quietly, startup checks `JWT_SECRET` and refuses to boot in production if it's absent or looks like a placeholder, surfacing as:

```
[FATAL] JWT_SECRET must be set to a secure random value in production!
```

This showed up during Railway deployment exactly as intended, and the fix was to set a real random `JWT_SECRET` env var, not to work around the check. The general shape (validate security-critical config once at startup, crash loudly rather than run in a bad state) is worth applying to any other secret that would be catastrophic to leave weak or unset, rather than discovering the gap later via an actual incident.
...
**Why:** widgets never call `ApiClient` directly for anything beyond the most trivial one-off requests, since a repository owns the URL paths and JSON parsing for its feature:
...
This keeps every screen's business logic focused on *when* to call something, not *how*, and it means the JSON shape only needs to be known in one place per feature.

### 2. `fromJson` Factory Constructor

**Where:** every class in `data/models/*.dart`

**Why:** consistent, defensive JSON parsing. MySQL returns numeric aggregates (`SUM()`, `DECIMAL` columns) as strings over the wire, so models never do a raw `as num` cast and instead go through small local helpers that accept either shape:
...
A direct `json['available'] as num` looks fine until the value happens to arrive as a string (which it does, from certain SQL aggregate queries), which has caused real crashes during development (see `ARCHITECTURE.md` §5.2), which is why every model funnels through a permissive helper like the one above instead.

### 3. `InheritedWidget` as a Pure DI Container

**Where:** `core/app_scope.dart`

**Why:** `AppScope` used to be described as standing "in place of a state-management package", but that's no longer accurate (see Pattern 4 below). Its role narrowed to just dependency injection: it hands out the `Session` and repositories that Cubits and screens construct themselves from, and holds no reactive app state of its own. `SubscriptionRepository` was removed from it entirely once `SubscriptionCubit` existed, keeping it there too would've just been a second, redundant path to the same thing. See `ARCHITECTURE.md` §4.2 for the full picture, including the `initState()` timing rule that every data-loading screen has to follow.
...
Marking one notification read on the notifications screen instantly updates every bell icon on screen, since they all `context.watch<NotificationCubit>()` the same instance, so no manual "reload after navigating back" is needed for anything this covers.

Every state class extends `Equatable` (from the `equatable` package) so `Cubit` can tell when a new state is *structurally* identical to the last one and skip an unnecessary rebuild, since without it `Cubit`'s default equality is reference-based, which would rebuild on every `emit()` even when nothing observable actually changed.

### 5. Scoped vs. App-Wide Cubits

**Where:** app-wide cubits are registered in `main.dart`'s `MultiBlocProvider` (all the Cubits listed in Pattern 4), and scoped ones are `chat/chat_cubit.dart`, `influencer/earnings/withdrawal_cubit.dart`

**Why:** not every Cubit needs to be reachable from everywhere. Chat and Withdrawal state is inherently tied to *one* open screen at a time, so there's no "keep multiple simultaneously-open screens in sync" problem to solve, so making them global would just be unnecessary ceremony. Instead, the screen itself is a thin wrapper that builds a fresh Cubit scoped to just that screen:
...
**What deliberately stayed on plain `setState`:** a few genuinely screen-local, non-duplicated bits of state were left alone rather than wrapped in a Cubit for its own sake, like `_AllCampaignsScreenState` (a private class inside `brand_dashboard_screen.dart`) keeps its own `_loading`/`_items` fields, since only that one screen ever needs them.
...
**Why:** before Cubits existed, the app had no shared/reactive state store at all, and a screen's data was just whatever it fetched on its own `_load()` call, so without explicitly reloading after a child screen popped, a dashboard's counts (e.g. "3 pending approvals") would stay stale until the whole tab was torn down and rebuilt. `main_screen.dart` uses an `IndexedStack` for the bottom-nav tabs specifically so they *don't* rebuild on every tab switch, which is what made this pattern necessary in the first place.

This is now **only needed in two situations**: (1) screens backed by a *scoped* Cubit (Chat, Withdrawal, see Pattern 5), since there's no global listener to react to changes automatically, and (2) the handful of plain-`setState` screens that were deliberately left alone (e.g. `_AllCampaignsScreenState`). Anywhere backed by an app-wide Cubit no longer needs this, since approving an applicant now updates the dashboard's counts reactively the moment `BrandDashboardCubit.load()` is called from wherever the approval happened, without the dashboard screen needing to be the one to trigger it.
...
| Fail-fast startup guard | Backend | `JWT_SECRET` validation at boot (production only) |
